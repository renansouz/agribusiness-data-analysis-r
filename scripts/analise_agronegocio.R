...
if (length(pacotes_faltando) > 0) {
  ...
  install.packages(pacotes_faltando, repos = "https://cran.r-project.org", dependencies = TRUE)
}
...
cat(sprintf("   Media    : %s toneladas\n", format(round(media, 2),   big.mark = ".", decimal.mark = ",")))
cat(sprintf("   Mediana  : %s toneladas\n", format(round(mediana, 2), big.mark = ".", decimal.mark = ",")))
cat(sprintf("   Moda     : aprox. %s toneladas (classe de 1.000 t)\n\n",
...
p1 <- ggplot(dados, aes(x = producao_toneladas / 1e6)) +
  ...
  geom_vline(xintercept = media   / 1e6, color = "#E53935", linewidth = 1, linetype = "dashed") +
  geom_vline(xintercept = mediana / 1e6, color = "#1565C0", linewidth = 1, linetype = "dotdash") +
  annotate("text", x = media/1e6   + 0.25, y = Inf, vjust = 2,   label = "Media",   color = "#E53935", size = 3.2) +
  annotate("text", x = mediana/1e6 - 0.25, y = Inf, vjust = 3.5, label = "Mediana", color = "#1565C0", size = 3.2) +
  labs(title    = "Histograma - Producao por Observacao",
       subtitle = "Linhas: Media (vermelha) e Mediana (azul)",
       x = "Producao (milhoes de toneladas)", y = "Frequencia") +
  tema_agro

p2 <- ggplot(dados, aes(x = "", y = producao_toneladas / 1e6)) +
  geom_boxplot(fill = "#A5D6A7", color = "#1B5E20", width = 0.5,
               outlier.color = "#E53935", outlier.size = 2.5) +
  stat_summary(fun = mean, geom = "point", shape = 18, size = 4, color = "#E53935") +
  labs(title    = "Boxplot - Producao (toneladas)",
       subtitle = "Losango vermelho = Media | Boxplot = Quartis",
       y = "Producao (milhoes de toneladas)", x = "") +
  tema_agro +
...
p3 <- ggplot(dados, aes(x = producao_toneladas / 1e6)) +
  ...
  geom_vline(xintercept = q1 / 1e6, linetype = "dashed", color = "#6A1B9A", linewidth = 0.8) +
  geom_vline(xintercept = q3 / 1e6, linetype = "dashed", color = "#6A1B9A", linewidth = 0.8) +
  annotate("text", x = q1/1e6, y = Inf, vjust = 2, hjust =  1.1, label = "Q1", color = "#6A1B9A", size = 3) +
  annotate("text", x = q3/1e6, y = Inf, vjust = 2, hjust = -0.1, label = "Q3", color = "#6A1B9A", size = 3) +
  labs(title    = "Curva de Densidade - Producao",
       subtitle = "Linhas tracejadas: Q1 e Q3",
       x = "Producao (milhoes de toneladas)", y = "Densidade") +
  tema_agro

p4 <- ggplot(dados, aes(sample = producao_toneladas)) +
  stat_qq(color = "#2E7D32", size = 2) +
  stat_qq_line(color = "#E53935", linewidth = 1) +
  labs(title    = "QQ-Plot - Normalidade",
...
p5 <- ggplot(freq_nivel, aes(x = nivel_tecnologico, y = n, fill = nivel_tecnologico)) +
  ...
  labs(title    = "Frequencia por Nivel Tecnologico",
       subtitle = "Variavel Qualitativa Ordinal - Adocao de Tecnologia no Campo",
       x = "Nivel Tecnologico", y = "Numero de Observacoes") +
  tema_agro
...
p6 <- ggplot(freq_nivel, aes(x = 2, y = n, fill = nivel_tecnologico)) +
  ...
  xlim(0.5, 2.5) +
  labs(title    = "Distribuicao Percentual - Nivel Tecnologico",
